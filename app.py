"""Application Streamlit — Détection de pneumonie par radiographie thoracique."""
...
def analyser_fichier(chemin: str):
    """Cache les résultats pour les images d'exemple (évite de recalculer à chaque interaction)."""
...
def afficher_resultat(image: Image.Image, resultat: dict, gradcam: Image.Image) -> None:
    """Affiche radio originale | Grad-CAM + probabilités."""
...
if _model is None:
    st.error(f"Modèle introuvable : `{MODEL_PATH.relative_to(BASE_DIR)}`")
    st.stop()
